	...
	public function saveDown(){
		...
		if($id === 'nw'){
			...
			$sql = "UPDATE ".$this->table."
					SET `name` = '".$down['meta']['name']."', `eventname` = '".$down['meta']['eventname']."', `data` = '".maybe_serialize($down)."', `offertext` = '".$html['offertext']."', `replacetext` = '".$html['replacetext']."'
					WHERE id = ".$id;
			$wpdb->query($sql);
			$re[] = $id;
		}
	...
	**/
	...
	public static function outputString( $down ){
		...
		<div class="countdown-details">
		  <div class="cd-date">
		    NEXT EVENT: ' . date("F j, Y", ($down['meta']['to'][1] / 1000)) . '
		  </div>
		  ' . $down['meta']['eventname'] . '
		</div>
		<div class="pbc-cover wpb-inline" data-pbc-setup="" data-countdown="'. htmlentities( json_encode($down) ) .'">'.
